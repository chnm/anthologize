...
class PdfAnthologizer extends Anthologizer {
    ...
    public $partH = '16';
    public $itemH = '12';
    public $headerLogo = 'jdh-logo-2.jpg'; //should be in /anthologize/images/
    ...
    public function init() {
       ...
       // $this->output->SetDefaultMonospacedFont(PDF_FONT_MONOSPACED);
        ...
        $this->output->SetDefaultMonospacedFont($font_family);
    ...
    public function appendFront() {
        ...
        $this->output->SetCreator("The Journal of Digital Humanities");
        ...
        $this->output->Image( ANTHOLOGIZE_IMAGES_PATH . "jdh_logo-full.jpg", '', '', '150', '', '', '', '', '', '', 'C');
        $this->output->SetY(80);
        $this->output->setFont($this->font_family, '', $this->baseH + 6);
        $this->output->Write('', $book_title, '', false, 'C', true );
        $this->output->setFont($this->font_family, '', $this->baseH);
        $this->output->Write('', "Published by the Roy Rosenzweig Center for History and New Media", '', false, 'C', true );
        $this->output->Write('', "Vol. 1 No. 1 Winter 2011", '', false, 'C', true );

        $this->output->SetY(100);
        $this->output->Write('', "Daniel J. Cohen, Editor", '', false, 'C', true );
        $this->output->Write('', "Joan Fragaszy Troyano, Editor", '', false, 'C', true );
        $this->output->Write('', "Sasha Hoffman, Associate Editor", '', false, 'C', true );
        $this->output->Write('', "Jeri Wieringa, Associate Editor", '', false, 'C', true );
        $this->output->Write('', "Daniel Ludington, Editorial Assistant", '', false, 'C', true );
        $this->output->SetY(130);
        $this->output->Write('', "Natalia Cecire, Guest Editor", '', false, 'C', true );

        $this->output->SetY(160);
        $this->output->Write('', "ISSN 2165-6673", '', false, 'C', true );
        $this->output->Write('', "CC-BY 3.0", '', false, 'C', true );
...
//dig up the author
$author = $this->api->getSectionPartItemAssertedAuthor($section, $partNo, $itemNo);

        if($section == 'body') {
            $this->output->Bookmark($title . " by " . $author, 1);
        }
